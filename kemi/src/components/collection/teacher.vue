<template>
  <div class="teach-text">
    <div class="teach-wrap" v-for="list in collectionList" :key="list.id" @click="goTeacherPage(list.id)">
      <div class="teach-img" :style="getLogoUrl(list.headImageUrl)"></div>
      <div class="teach-cont">
        <div class="teach-title">
          <text class="teach-name">{{list.userName}}</text>
          <text class="teach-time">{{list.createTime}}</text>
        </div>
        <div><text class="teach-font" lines="1">{{list.describe1}}</text></div>
        <div class="teach-introduction">
          <text class="teach-font">简介：</text>
          <text class="teach-font teachBox" lines="1" v-html="list.introduction"></text>
        </div>
      </div>
    </div>
      ...
      getActivityList() {
        if (!this.showLoading) {
          ...
          stream.fetch({
            ...
            body: `{"page": "${this.page}", "size": "${this.size}", "collectionType": "2"}`,
          }, res => {
            if (res.ok) {
