<template>
  <div class="main-content">
    <div class="content-info" v-for="list in collectionList" :key="list.id" @click="goContPage(list.id, list.contentType)">
      <div class="title">
        <div><text class="title-right">{{list.createTime}}</text></div>
        <div class="title-left">
          <text class="title-logo" :style="getLogoUrl(list.parentLogo)"></text>
          <text class="title-name" lines="1">{{list.parentName}}</text>
        </div>
      </div>
      <div class="content-warp">
        <div class="content-image" :style="getLogoUrl(list.imageUrl)" ></div>
        <div class="content-cont">
          <text class="content-name" lines="2">{{list.contentName}}</text>
          <text class="activity">{{collectionType[list.contentType]}}</text>
        </div>
      </div>
    </div>
    <div class="no-article-container" v-if="collectionList.length <= 0">
      ...
      <text class="noArticleTxt">还没有收藏~</text>
    </div>
  </div>
</template>
<style scoped>
  ...
  module.exports = {
    ...
    data() {
      return {
        ...
        showLoading: false,
        collectionList: [],
        collectionType: ["活动", "详情"],
      };
    },
    methods: {
      goContPage(activityID, type) {
        console.log(type);
        if (!type) {
          this.$router.push({
            name: "activityInfo",
      ...
      getLogoUrl(url) {
        return `background-image: url('${Vue.filter("dirImage")(url)}')`;
      },
      ...
      getActivityList() {
        if (!this.showLoading) {
          ...
          stream.fetch({
            method: 'POST',
            type: 'json',
            url: `${__APIDIR}appapi/collection`,
            headers: {
              "Content-Type": "application/json;charset=utf-8",
              "token": __userToken,
            },
            body: `{"page": "${this.page}", "size": "${this.size}", "collectionType": "1"}`,
          }, res => {
            if (res.ok) {
              ...
              if (res.data.code === "1") {
                ...
                if (res.data.result.length) {
                  this.page++;
                  if (this.page) {
                    this.collectionList = this.collectionList.concat(res.data.result);
                  } else {
