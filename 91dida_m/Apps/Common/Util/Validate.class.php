...
{		
    ...
    {
        return (bool)preg_match('/^((\d{3,4})|\d{3,4}-)?\d{7,8}(-\d+)*$|^400\d?(-\d{3,4}){2}$/i',$str);//preg_match('/^((\d{3,4})|\d{3,4}-)?\d{7,8}(-\d+)*$/i',$str);
    }
    /**
     * @brief 手机号码验证
     * @param string $str
    ...
    public static function mobi($str='')
    {
        ...
        // 更改验证方式，20160704 liqinghua
		
		return (bool)preg_match("/1\d{10}$/",$str);
		
        //限制手机号的
       // return (bool)preg_match('/(13\d|14[57]|15[^4,\D]|17[678]|18\d)\d{8}|170[059]\d{7}/', $str);
