<template>
  <div class="mech-text">
    <div class="mech-wrap" v-for="list in collectionList" :key="list.id" @click="goMechaPage(list.id)">
      <div class="mech-img" :style="getLogoUrl(list.logoUrl)" ></div>
      <div class="mech-cont">
        <div class="mech-title">
          <text class="mech-name" lines="1">{{list.businessName}}</text>
          <text class="mech-time">{{list.createTime}}</text>
        </div>
        <div><text class="mech-teacher">{{list.serviceItems}}</text></div>
        <div class="mech-font">
          <text class="mech-teacher">{{list.teacherNumber}}名老师</text>
          <text class="mech-teacher">{{list.commentNumber}}条评价</text>
        </div>
      </div>
    </div>
      ...
      getActivityList() {
        if (!this.showLoading) {
          ...
          stream.fetch({
            ...
            body: `{"page": "${this.page}", "size": "${this.size}", "collectionType": "3"}`,
          }, res => {
            if (res.ok) {
