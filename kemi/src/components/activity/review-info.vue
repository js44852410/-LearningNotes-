    ...
    methods: {
      getPageInfo() {
        ...
        stream.fetch({
          method: 'post',
          type: 'json',
          url: `${__APIDIR}appapi/historyActivity/${this.id}`,
          headers: {
      ...
      getReadingQuantity() {
        stream.fetch({
          method: 'POST',
          type: 'json',
          url: `${__APIDIR}appapi/readAct`,
          headers: {
            "Content-Type": "application/json;charset=utf-8",
          },
          body: '{"activityId": "' + this.id + '"}',
        }, res => {
          if (res.ok) {
            let data = res.data || {};
      ...
      appreciate() {
        stream.fetch({
          method: 'POST',
          type: 'json',
          url: `${__APIDIR}appapi/userActivity/praise`,
          headers: {
            "Content-Type": "application/json;charset=utf-8",
          },
          ...
          if (res.ok) {
            this.appreciateIt = !this.appreciateIt;
          }
        });
      },
    },
