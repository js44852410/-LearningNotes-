<template>
  <list class="list-container" @loadmore="getNewsList" :loadmoreoffset="loadmoreoffset">
    <cell class="news-item" v-for="item in newsList" :key="item.id" @click="goPage(item)">
      <div class="news-img-container" :style="{backgroundImage: getBgImg(item)}">
        <text class="unread-icon" v-if="item.status === '0'"></text>
      </div>
      <div class="news-mes-container">
        <text class="news-title" lines="1">{{item.detail}}</text>
        <text class="news-time">{{item.createTime}}</text>
      </div>
    </cell>
    <cell class="shownoAct" v-if="newsList.length == 0">
      ...
      <text class="noActiveTxt">暂无消息</text>
    </cell>
  </list>
</template>
<style scoped>
...
<script>
  ...
  export default {
    ...
    methods: {
      getBgImg(item) {
        return `url(${Vue.filter("defaultHead")(item.image)})`;
      },
      ...
      getNewsList() {
        if (!this.ajaxFlag) {
          ...
          stream.fetch({
            method: 'POST',
            type: 'json',
            url: `${__APIDIR}appapi/message/getPageMessage`,
            headers: {
              "Content-Type": "application/json;charset=utf-8",
              "token": __userToken,
            },
            body: `{"page": ${this.page++}, "size": 10}`,
          }, res => {
            this.ajaxFlag = false;
            if (res.data.code === "1") {
      ...
      goPage(item) {
        stream.fetch({
          method: 'POST',
          type: 'json',
          url: `${__APIDIR}appapi/message/editStatus`,
          headers: {
            "Content-Type": "application/json;charset=utf-8",
            "token": __userToken,
          },
