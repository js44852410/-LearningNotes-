...
class Transport
{
     ...
     *
     * @access  public
     * @param   integer     $time_limit  脚本执行时间。－1表示采用PHP的默认值。
     * @param   integer     $connect_timeout  在多少秒之内，如果连接不可用，脚本就停止连接。－1表示采用PHP的默认值。
     * @param   integer     $stream_timeout  连接后，限定多少秒超时。－1表示采用PHP的默认值。此项仅当采用CURL库时启用。
     * @param   boolean     $use_curl 是否使用CURL库来连接。false表示采用fsockopen进行连接。
     * @return  void
     */
     ...
     * @return  void
     */
    function transport($time_limit = -1, $connect_timeout = -1, $stream_timeout = -1, $use_curl = true)
    {
     ...
     * @return  array                       成功返回一维关联数组，形如array('header'=>'返回的http头部信息', 'body'=>'返回的内容')，
