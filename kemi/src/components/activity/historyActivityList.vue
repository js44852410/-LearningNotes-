  ...
  <list>
    <cell class="activity_container" v-for="item in activityList" :key="item.activityId"
      ...
      <div class="activity_mes_container">
        ...
        <text class="sign-up-count">报名人数 {{item.count}}</text>
        <text class="comment-count">评价 {{item.commentNumber}}</text>
      </div>
    </cell>
    ...
    <cell v-if="activityList.length <= 0 && size >= 5" class="no-activity-container">
      ...
      <text class="noActiveTxt">暂无活动</text>
    </cell>
  </list>
</template>
      ...
      getActivityList() {
        if (!this.showLoading) {
          ...
          stream.fetch({
            method: 'POST',
            type: 'json',
            url: `${__APIDIR}appapi/getHisActPage`,
            headers: {
              "Content-Type": "application/json;charset=utf-8",
            },
            body: `{"page": "${this.page}", "size": "${this.size}", "${this.keyword}": "${this.id}"}`,
          }, res => {
            if (res.ok) {
